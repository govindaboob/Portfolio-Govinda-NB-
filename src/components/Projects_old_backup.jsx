...
export default function Projects() {
  ...
  useEffect(() => {
    // Check if we just came back from a project details page
    if (sessionStorage.getItem('fromProjectDetails') === 'true') {
      sessionStorage.removeItem('fromProjectDetails')
    ...
    const handlePopState = (event) => {
      // Mark that we're coming back from project details
      sessionStorage.setItem('fromProjectDetails', 'true')
      // Reload to restore React app
  ...
  const openProjectDetails = (project) => {
    ...
    // Create a clean project ID for the URL
    const projectId = (project.id || project.title.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, ''))
    
    // Push a new state to browser history with proper state management
    const newState = { page: 'project', projectId: projectId, projectTitle: project.title }
